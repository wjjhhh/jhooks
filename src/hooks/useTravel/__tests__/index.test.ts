...
describe('useTravel', () => {
  ...
  it('go should work for back step', () => {
  ...
  it('back and forward should work after overwrite later step', () => {
    ...
    act(() => {
      hook.result.current.setValue({ a: 2, b: 1 }, { overwrite: true });
    });
    act(() => {
      hook.result.current.setValue({ a: 2, b: 2 }, { overwrite: true });
    });
    act(() => {
  ...
  it('back and forward should work with simple type after overwrite later step', () => {
    const hook = renderHook(() => useTravel(''));
    act(() => {
  ...
  it('should work forward and back after overwrite', () => {
    const hook = renderHook(() => useTravel(''));
    act(() => {
      hook.result.current.setValue('1');
    });
